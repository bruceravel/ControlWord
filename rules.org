...
Control Word is a tile laying word game much like a popular crossword
board game, but with elements of area control and direct player
interaction.  Each player plays words using a different color of
tiles.  Points are scored by having the most tiles of your color in a
word at the end of the game.  Longer words score more points.  The
...
that each player will draw to at the end of the round.  /Set die on
the round counter.../

/{{{hl(Roll a die to set how many letters can be used in the replacement phase?)}}}/
 ...
 * The player with the most tiles (simple plurality) in a word wins
   ...
   controls the word and receives {{{hl(4 points)}}} at game's end:
   ...
   controls the word by plurality and receives {{{hl(14 points)}}} at
   game's end:
...
- Award {{{hl(4 bonus point)}}} bonus for each J, Q, X, or Z to the
  person controlling that word.
